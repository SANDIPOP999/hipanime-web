...
<head>
   <title>Hipanime - Official Hipanime #1 Watch High Quality Anime Online Without Ads</title>
    ...
    <meta property="og:title" content="Hipanime - Official Hipanime #1 Watch High Quality Anime Online Without Ads">
    <meta property="og:description" content="Hipanime - Official Hipanime #1 Watch High Quality Anime Online Without Ads. You can watch anime online free in HD without Ads. Best place for free find and one-click anime.">
...
<section class="block_area block_area_home">
    <div class="block_area-header">
        <div class="float-left bah-heading mr-4">
            <h2 class="cat-heading">Most Populars</h2>
        </div>
        <div class="float-right viewmore">
            <a class="btn" href="/most-popular">View more<i class="fas fa-angle-right ml-2"></i></a>
        </div>
        <div class="clearfix"></div>
    </div>
    <div class="tab-content">
        <div class="block_area-content block_area-list film_list film_list-grid">
            <div class="film_list-wrap">
                <?php 
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, "$ani/meta/popular");
                ...
                foreach ((array) $json as $movie) { 
                    // Extract only "G", "R", or "PG-13" from the rating
                    preg_match('/^(G|R|PG-13)/', $movie['rating'], $matches);
                    $mainRating = isset($matches[1]) ? $matches[1] : "N/A"; // Default if not found

                    $episodeNum = $movie['total_episodes'] ?? "1"; // Use total episodes directly
                ?>
                    <div class="flw-item">
                        <div class="film-poster">
                            <div class="tick ltr">
                                <div class="tick-item-dub tick-eps amp-algn"><?= $mainRating ?></div> <!-- Shows only G, R, PG-13 -->
                            </div>
                            <div class="tick rtl">
                                <div class="tick-item tick-eps amp-algn">Episode <?= $episodeNum ?></div> <!-- Shows total episodes -->
                            </div>
                            <img class="film-poster-img lazyload"
                                data-src="https://ik.imagekit.io/<?= $imgk ?>/tr:f-webp/<?= $movie['cover_image'] ?>"
                                src="https://ik.imagekit.io/<?= $imgk ?>/tr:f-webp/<?= $movie['cover_image'] ?>"
                                alt="Watch free online <?= htmlspecialchars($movie['title']) ?> on Hipanime">
                            <a class="film-poster-ahref"
                                href="/anime/<?= $movie['mal_id'] ?>" title="<?= htmlspecialchars($movie['title']) ?>"
                                data-jname="<?= htmlspecialchars($movie['title']) ?>"><i class="fas fa-play"></i></a>
                        </div>
                        <div class="film-detail">
                            <h3 class="film-name">
                                <a href="/anime/<?= $movie['mal_id'] ?>" title="<?= htmlspecialchars($movie['title']) ?>"
                                   data-jname="<?= htmlspecialchars($movie['title']) ?>"><?= htmlspecialchars($movie['title']) ?></a>
                            </h3>
                            <div class="fd-infor">
